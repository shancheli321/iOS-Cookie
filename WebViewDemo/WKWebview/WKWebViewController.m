...
//    configuration.processPool = pool;
    ...
    // 加cookie给h5识别，表明在ios端打开该地址  // 自定义脚本等
    ...
    // 2、在创建的时候存放到WKUserScript中进行添加cookie
    
    //添加自定义的cookie
    NSString *cookieValue = @"document.cookie = 'fromapp=ios';document.cookie = 'channel=appstore'; document.cookie = 'DarkAngelCookie=DarkAngel;";
    WKUserScript *newCookieScript = [[WKUserScript alloc] initWithSource:cookieValue injectionTime:WKUserScriptInjectionTimeAtDocumentStart forMainFrameOnly:NO];
